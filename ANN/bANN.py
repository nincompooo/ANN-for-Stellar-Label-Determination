...
def train_and_evaluate():
    ...
    wavelength = np.loadtxt("Koi1422_HET.txt")[:, 0]
    # X, mask = enforce_masking(X_full, wavelength)
    ...
    val_loader   = DataLoader(StellarDataset(X_val, y_val), batch_size=128)

    device = "cuda" if torch.cuda.is_available() else "cpu"

    model = StellarANN(X.shape[1], len(label_names)).to(device)
    ...
    print("\nMSE per parameter:")
    for name, val in zip(label_names, ((y_pred - y_true) ** 2).mean(axis=0)):
        print(f"{name:10s}: {val:.4f}")

    # PLOTS
    plot_mse(output_dir, train_losses, val_losses)
